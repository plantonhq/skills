...
| `spec.rules` | `[]KubernetesTcpRouteRule` | yes |  |  |
...
`[]KubernetesTcpRouteRule` · required
...
Reference an output from another manifest as `valueFrom: {kind: KubernetesTcpRoute, name: <resource-name>, fieldPath: status.outputs.<output>}`.

| Output | Type | Description |
|---|---|---|
| `status.outputs.route_name` | `string` | Name of the created TCPRoute (equals metadata.name). In InfraCharts this orders the route after the Gateway and backends it references. |
| `status.outputs.namespace` | `string` | Namespace the TCPRoute was created in (the resolved spec.namespace). Cross-namespace parent and backend references from this route are subject to ReferenceGrant rules relative to this value. |
