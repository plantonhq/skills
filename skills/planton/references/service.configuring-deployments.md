...
- **Config references stay unresolved.** `$var/...` and `$secret/...` values in manifests resolve just-in-time at deploy; the declaration never carries secret material. A secret reference must sit in the target's secret home — `env.secrets` on a Kubernetes workload, an env entry's `secretValue` on Cloud Run, `secretEnvironment` on an ECS task definition — never in the plain field beside it, which every viewer of the running resource reads. Saving such a configuration is accepted; the next deploy is refused before any of the environment's manifests applies, and the run's environment row names the manifest (number, kind and name), the field, the variable and the field to move it to (`references/infra.config-references.md`, "Where a secret reference may go").
...
2. **Nothing deploys when you save.** Configuration changes take effect on the next deployment that reads the current configuration — the next push or manual run, or a CLI deploy. A promotion does not: it carries the configuration captured with the artifact it moves (`references/service.delivery-verbs.md`). Say this every time: a person who saves and sees nothing change will otherwise read success as failure. (A rollback does NOT pick up new configuration — it re-applies a captured pair; see `references/service.delivery-verbs.md`.)
